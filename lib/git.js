...
async function getModifiedFiles(execaOptions) {
  return (await execa('git', ['ls-files', '-m', '-o', '-d', '--exclude-standard'], execaOptions)).stdout
    .split('\n')
    .map(file => file.trim())
...
async function add(files, execaOptions) {
  const shell = await execa('git', ['add', ...files], {...execaOptions, reject: false});
...
async function push(origin, branch, execaOptions) {
  await execa('git', ['push', '--follow-tags', origin, `HEAD:${branch}`], execaOptions);
}
